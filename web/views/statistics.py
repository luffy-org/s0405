...
def statistics_project_user(request, project_id):
    ...
    all_user_dict[request.tracer.project.creator.id] = {
        ...
        'status': {item[0]: 0 for item in models.Issues.status_choice}
    }
    all_user_dict[None] = {
        'name': '未指派',
        'status': {item[0]: 0 for item in models.Issues.status_choice}
    ...
    for item in models.Issues.status_choice:
    ...
    for key, text in models.Issues.status_choice:
        ...
        for row in all_user_dict.values():
            ...
            data_result_dict[key].get('data').append(count)
